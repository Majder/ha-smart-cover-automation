...
async def test_async_setup_entry_creates_all_numbers(mock_coordinator_basic: DataUpdateCoordinator) -> None:
    ...
    # Verify we have exactly 8 entities
    assert len(entities_list) == 8
    ...
    assert isinstance(entities_list[5], SunElevationMaxHysteresisNumber)
    assert isinstance(entities_list[6], DailyMaxTemperatureThresholdNumber)
    assert isinstance(entities_list[7], DailyMinTemperatureThresholdNumber)
...
async def test_async_setup_entry_with_real_hass_instance(
    ...
    assert isinstance(captured[5], SunElevationMaxHysteresisNumber)
    assert isinstance(captured[6], DailyMaxTemperatureThresholdNumber)
    assert isinstance(captured[7], DailyMinTemperatureThresholdNumber)
...
async def test_async_setup_entry_adds_external_tilt_numbers_when_modes_external(mock_coordinator_basic: DataUpdateCoordinator) -> None:
    ...
    assert isinstance(captured[8], GlobalExternalTiltDayNumber)
    assert isinstance(captured[9], GlobalExternalTiltNightNumber)
    assert isinstance(captured[10], CoverExternalTiltDayNumber)
    assert isinstance(captured[11], CoverExternalTiltNightNumber)
